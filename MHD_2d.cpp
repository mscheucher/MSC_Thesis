...
int kx;					//Wavenumber of Boundary Layer disturbance
...
//   ofstream inpvar ("inpvar.txt");	//control output of input file
//   for (int i=0;i<18;++i) inpvar<<i<<" "<<variables[i]<<endl;
...
//   init_rho(u[0]);   init_vx(w[0]);  init_p(w[3]);  init_Bx(u[5]);  init_By(u[6]);  init_Bz(u[7]); 
  ...
  initial.open ("data/initial_MHD_u0.txt");
  for (int j=2;j<nx+2;++j){ 
    for (int i=2;i<ny+2;++i) initial<<u[0][j][i]<<" ";
    initial<<endl;
  }
  initial.close();
  initial.open ("data/initial_MHD_u1.txt");
  for (int j=2;j<nx+2;++j){ 
    for (int i=2;i<ny+2;++i) initial<<u[1][j][i]<<" ";
    initial<<endl;
  }
  initial.close();
  initial.open ("data/initial_MHD_u2.txt");
  for (int j=2;j<nx+2;++j){ 
    for (int i=2;i<ny+2;++i) initial<<u[2][j][i]<<" ";
    initial<<endl;
  }
  initial.close();  
  initial.open ("data/initial_MHD_u3.txt");
  for (int j=2;j<nx+2;++j){ 
    for (int i=2;i<ny+2;++i) initial<<u[3][j][i]<<" ";
    initial<<endl;
  }  
  initial.close();
  initial.open ("data/initial_MHD_u4.txt");
  for (int j=2;j<nx+2;++j){ 
    for (int i=2;i<ny+2;++i) initial<<u[4][j][i]<<" ";
    initial<<endl;
  }
  initial.close();
  initial.open ("data/initial_MHD_u5.txt");
  for (int j=2;j<nx+2;++j){ 
    for (int i=2;i<ny+2;++i) initial<<u[5][j][i]<<" ";
    initial<<endl;
  }
  initial.close();  
  initial.open ("data/initial_MHD_u6.txt");
  for (int j=2;j<nx+2;++j){ 
    for (int i=2;i<ny+2;++i) initial<<u[6][j][i]<<" ";
    initial<<endl;
  }
  initial.close();  
  initial.open ("data/initial_MHD_u7.txt");
  for (int j=2;j<nx+2;++j){ 
    for (int i=2;i<ny+2;++i) initial<<u[7][j][i]<<" ";
    initial<<endl;
  }  
  initial.close();
  initial.open ("data/initial_MHD_w0.txt");
  for (int j=2;j<nx+2;++j){ 
    for (int i=2;i<ny+2;++i) initial<<w[0][j][i]<<" ";
    initial<<endl;
  }
  initial.close();
  initial.open ("data/initial_MHD_w1.txt");
  for (int j=2;j<nx+2;++j){
    for (int i=2;i<ny+2;++i) initial<<w[1][j][i]<<" ";
    initial<<endl;
  }
  initial.close();  
  initial.open ("data/initial_MHD_w2.txt");
  for (int j=2;j<nx+2;++j){ 
    for (int i=2;i<ny+2;++i) initial<<w[2][j][i]<<" ";
    initial<<endl;
  }
  initial.close();  
  initial.open ("data/initial_MHD_w3.txt");
  for (int j=2;j<nx+2;++j){ 
    for (int i=2;i<ny+2;++i) initial<<w[3][j][i]<<" ";
    initial<<endl;
  }  
  initial.close();
  initial.open ("data/initial_MHD_w4.txt");
  for (int j=2;j<nx+2;++j){ 
    for (int i=2;i<ny+2;++i) initial<<w[4][j][i]<<" ";
    initial<<endl;
  }  
  initial.close();
...
//---------- main loops: calling main functions and write files ----------
  ...
  for (double t=0.;t<=tmax;t+=2.*dt){
    ...
    if (pmin < plim){
    	data.open (std::string("data/pressure_problem_t" + doubleToString(t) +"_pmin_" + intToString(pmin) + ".txt").c_str());
	for (int j=2;j<nx+2;++j){ 
...
//     	data.open (std::string("data/data_MHD_u0_t" + doubleToString(t) +"_n" + intToString(nx) + ".txt").c_str());
// 	for (int j=2;j<nx+2;++j){ 
// 	  for (int i=2;i<ny+2;++i) data<<u[0][j][i]<<" ";
// 	  data<<endl;
// 	}
// 	data.close();
...
//         cout << "t= "<< t << "	dt= " << dt << " writing fluxes, etc..." << endl;    
	...
	data.close();
	data.open (std::string("data/data_MHD_u1_t" + doubleToString(t) +"_n" + intToString(nx) + ".txt").c_str());
	for (int j=2;j<nx+2;++j){ 
	  for (int i=2;i<ny+2;++i) data<<u[1][j][i]<<" ";
	  data<<endl;
	}
	data.close();
	data.open (std::string("data/data_MHD_u2_t" + doubleToString(t) +"_n" + intToString(nx) + ".txt").c_str());
	for (int j=2;j<nx+2;++j){ 
	  for (int i=2;i<ny+2;++i) data<<u[2][j][i]<<" ";
	  data<<endl;
	}
	data.close();
	data.open (std::string("data/data_MHD_u3_t" + doubleToString(t) +"_n" + intToString(nx) + ".txt").c_str());
	for (int j=2;j<nx+2;++j){ 
	  for (int i=2;i<ny+2;++i) data<<u[3][j][i]<<" ";
	  data<<endl;
	}
	data.close();
	data.open (std::string("data/data_MHD_u4_t" + doubleToString(t) +"_n" + intToString(nx) + ".txt").c_str());
	for (int j=2;j<nx+2;++j){ 
	  for (int i=2;i<ny+2;++i) data<<u[4][j][i]<<" ";
	  data<<endl;
	}
	data.close();
	data.open (std::string("data/data_MHD_u5_t" + doubleToString(t) +"_n" + intToString(nx) + ".txt").c_str());
	for (int j=2;j<nx+2;++j){ 
	  for (int i=2;i<ny+2;++i) data<<u[5][j][i]<<" ";
	  data<<endl;
	}
	data.close();
	data.open (std::string("data/data_MHD_u6_t" + doubleToString(t) +"_n" + intToString(nx) + ".txt").c_str());
	for (int j=2;j<nx+2;++j){ 
	  for (int i=2;i<ny+2;++i) data<<u[6][j][i]<<" ";
	  data<<endl;
	}
	data.close();
	data.open (std::string("data/data_MHD_u7_t" + doubleToString(t) +"_n" + intToString(nx) + ".txt").c_str());
	for (int j=2;j<nx+2;++j){ 
	  for (int i=2;i<ny+2;++i) data<<u[7][j][i]<<" ";
	  data<<endl;
	}
	data.close();
	data.open (std::string("data/data_MHD_w0_t" + doubleToString(t) +"_n" + intToString(nx) + ".txt").c_str());
	for (int j=2;j<nx+2;++j){ 
	  for (int i=2;i<ny+2;++i) data<<w[0][j][i]<<" ";
	  data<<endl;
	}
	data.close();
	data.open (std::string("data/data_MHD_w1_t" + doubleToString(t) +"_n" + intToString(nx) + ".txt").c_str());
	for (int j=2;j<nx+2;++j){ 
	  for (int i=2;i<ny+2;++i) data<<w[1][j][i]<<" ";
	  data<<endl;
	}
	data.close();
	data.open (std::string("data/data_MHD_w2_t" + doubleToString(t) +"_n" + intToString(nx) + ".txt").c_str());
	for (int j=2;j<nx+2;++j){ 
	  for (int i=2;i<ny+2;++i) data<<w[2][j][i]<<" ";
	  data<<endl;
	}
	data.close();
	data.open (std::string("data/data_MHD_w3_t" + doubleToString(t) +"_n" + intToString(nx) + ".txt").c_str());
	for (int j=2;j<nx+2;++j){ 
	  for (int i=2;i<ny+2;++i) data<<w[3][j][i]<<" ";
	  data<<endl;
	}
	data.close();
	data.open (std::string("data/data_MHD_w4_t" + doubleToString(t) +"_n" + intToString(nx) + ".txt").c_str());
	for (int j=2;j<nx+2;++j){ 
	  for (int i=2;i<ny+2;++i) data<<w[4][j][i]<<" ";
	  data<<endl;
	}
	data.close();
    }
  ...
  data.open (std::string("data/data_MHD_u0_t" + doubleToString(tmax) +"_n" + intToString(nx) + ".txt").c_str());
  for (int j=2;j<nx+2;++j){ 
    for (int i=2;i<ny+2;++i) data<<u[0][j][i]<<" ";
    data<<endl;
  }
  data.close();
  data.open (std::string("data/data_MHD_u1_t" + doubleToString(tmax) +"_n" + intToString(nx) + ".txt").c_str());
  for (int j=2;j<nx+2;++j){ 
    for (int i=2;i<ny+2;++i) data<<u[1][j][i]<<" ";
    data<<endl;
  }
  data.close();
  data.open (std::string("data/data_MHD_u2_t" + doubleToString(tmax) +"_n" + intToString(nx) + ".txt").c_str());
  for (int j=2;j<nx+2;++j){ 
    for (int i=2;i<ny+2;++i) data<<u[2][j][i]<<" ";
    data<<endl;
  }
  data.close();  
  data.open (std::string("data/data_MHD_u3_t" + doubleToString(tmax) +"_n" + intToString(nx) + ".txt").c_str());
  for (int j=2;j<nx+2;++j){ 
    for (int i=2;i<ny+2;++i) data<<u[3][j][i]<<" ";
    data<<endl;
  }
  data.close();  
  data.open (std::string("data/data_MHD_u4_t" + doubleToString(tmax) +"_n" + intToString(nx) + ".txt").c_str()); 
  for (int j=2;j<nx+2;++j){
    for (int i=2;i<ny+2;++i) data<<u[4][j][i]<<" ";
    data<<endl;
  }
  data.close();
  data.open (std::string("data/data_MHD_u5_t" + doubleToString(tmax) +"_n" + intToString(nx) + ".txt").c_str()); 
  for (int j=2;j<nx+2;++j){
    for (int i=2;i<ny+2;++i) data<<u[5][j][i]<<" ";
    data<<endl;
  }
  data.close();
  data.open (std::string("data/data_MHD_u6_t" + doubleToString(tmax) +"_n" + intToString(nx) + ".txt").c_str()); 
  for (int j=2;j<nx+2;++j){
    for (int i=2;i<ny+2;++i) data<<u[6][j][i]<<" ";
    data<<endl;
  }
  data.close();
  data.open (std::string("data/data_MHD_u7_t" + doubleToString(tmax) +"_n" + intToString(nx) + ".txt").c_str()); 
  for (int j=2;j<nx+2;++j){
    for (int i=2;i<ny+2;++i) data<<u[7][j][i]<<" ";
    data<<endl;
  }
  data.close();
	data.open (std::string("data/data_MHD_w0_t" + doubleToString(tmax) +"_n" + intToString(nx) + ".txt").c_str());
	for (int j=2;j<nx+2;++j){ 
	  for (int i=2;i<ny+2;++i) data<<w[0][j][i]<<" ";
	  data<<endl;
	}
	data.close();
	data.open (std::string("data/data_MHD_w1_t" + doubleToString(tmax) +"_n" + intToString(nx) + ".txt").c_str());
	for (int j=2;j<nx+2;++j){ 
	  for (int i=2;i<ny+2;++i) data<<w[1][j][i]<<" ";
	  data<<endl;
	}
	data.close();
	data.open (std::string("data/data_MHD_w2_t" + doubleToString(tmax) +"_n" + intToString(nx) + ".txt").c_str());
	for (int j=2;j<nx+2;++j){ 
	  for (int i=2;i<ny+2;++i) data<<w[2][j][i]<<" ";
	  data<<endl;
	}
	data.close();
	data.open (std::string("data/data_MHD_w3_t" + doubleToString(tmax) +"_n" + intToString(nx) + ".txt").c_str());
	for (int j=2;j<nx+2;++j){ 
	  for (int i=2;i<ny+2;++i) data<<w[3][j][i]<<" ";
	  data<<endl;
	}
	data.close();
	data.open (std::string("data/data_MHD_w4_t" + doubleToString(tmax) +"_n" + intToString(nx) + ".txt").c_str());
	for (int j=2;j<nx+2;++j){ 
	  for (int i=2;i<ny+2;++i) data<<w[4][j][i]<<" ";
	  data<<endl;
	}
...
//------ NEGATIVE PRESSURE CORRECTION --------------------------------------------------   
  ...
  if (correcting==1){
	ofstream data;
    	data.open (std::string("data/pressure_problem_t" + doubleToString(t) +".txt").c_str());
	for (int j=2;j<nx+2;++j){ 
	  for (int i=2;i<ny+2;++i) data<<w[3][j][i]<<" ";
	  data<<endl;
	}
